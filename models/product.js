...
export class ProductModel {
  ...
  static async searchAll ({ q, limit, offset }) {
    const query = `
      SELECT
        id, nombre, codigo_barra, imagen_url, detalle_nutricional, categoria_id, marca, url_producto
      FROM productos
      ...
      LIMIT $2 OFFSET $3;
    `
    const result = await executeQuery(query, [q, limit, offset])
    return result.records
  }

  static async searchByCategoryId ({ categoryId, q, marca, limit, offset }) {
    // si quieres mantener "marca exacta" como hoy:
  ...
  static async searchByCategoryChildOf ({ parentId, q, marca, limit, offset }) {
    ...
    const query = `
      SELECT
        p.id, p.nombre, p.codigo_barra, p.imagen_url, p.detalle_nutricional, p.categoria_id, p.marca, p.url_producto
      FROM categorias AS c
      JOIN productos  AS p ON p.categoria_id = c.id
      WHERE c.parent_id = $1
  ...
  static async searchByMarca ({ marca, q, limit, offset }) {
    const query = `
      ...
      LIMIT $3 OFFSET $4;
    `
    const result = await executeQuery(query, [marca, q, limit, offset])
    return result.records
  }
}
